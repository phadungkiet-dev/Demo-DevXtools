...
export function CommandMenu() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setIsMounted(true); // Set เป็น true เมื่ออยู่ฝั่ง Client แล้ว
    ...
    const down = (e: KeyboardEvent) => {
      if (e.key === "k" && (e.metaKey || e.ctrlKey)) {
        // metaKey = Cmd (Mac), ctrlKey = Ctrl (Windows)
    ...
    return () => {
      clearTimeout(timer);
      document.removeEventListener("keydown", down); // Clean up เมื่อ component หายไป (ป้องกัน Memory Leak)
    };
  }, []);
  ...
  return (
    <>
      ...
      {/* แสดง Dialog เฉพาะเมื่อ isMounted เป็น true เท่านั้น */}
      {isMounted && (
